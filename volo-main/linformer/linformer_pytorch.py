...
class MHAttention(nn.Module):
    ...
    def forward(self, tensor, **kwargs):
        batch_size, input_seq, channels = tensor.shape
        assert not (self.decoder_mode and "embeddings" not in kwargs), "Embeddings must be supplied if decoding"
        assert not ("embeddings" in kwargs and (kwargs["embeddings"].shape[0], kwargs["embeddings"].shape[1], kwargs["embeddings"].shape[2]) != (batch_size, input_seq, channels)), "Embeddings size must be the same as the input tensor"
        head_outputs = []
...
class Linformer(nn.Module):
    ...
    def forward(self, tensor, **kwargs):
        ...
        <= Input is (batch_size, height, weight, channels)
        """
        d = tensor.shape

        print(d)
        if(len(d)>3):
            tensor = tensor.reshape(d[0], d[1] * d[2], d[3])
        bt, s, c = tensor.shape
        assert s == self.input_size, "This tensor is of the wrong size. Dimension 1 has to match the `input_size` flag"
